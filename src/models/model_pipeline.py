...
import pandas as pd
import numpy as np  # <--- ADDED for correlation-based filtering
...
class ModelPipeline:
    ...
    # ---------------------------------------------------------------------
    # Automated correlation-based feature filtering
    # ---------------------------------------------------------------------
    def _filter_correlated_features(self, X_df: pd.DataFrame, threshold: float = 0.90) -> pd.DataFrame:
        """
        Removes features that have a correlation above 'threshold' with any other feature
        ...
        if to_drop:
            self.logger.info(f"Removing {len(to_drop)} highly correlated features (corr > {threshold}): {to_drop}")
    ...
    def train_on_horizons(
        ...
        for idx, combo in enumerate(tqdm(combos, desc="Training across horizons", unit="horizon")):
            ...
            # Drop future horizon columns.
            X_f = self._drop_future_horizon_columns(X_f, gather)

            # Skip if target changes are too small overall.
            ...
            X_f_numeric = self._filter_correlated_features(X_f_numeric, threshold=0.90)
            if X_f_numeric.shape[1] == 0:
                self.logger.warning(f"All features removed after correlation filtering. Skipping horizon {gather} -> {predict}.")
                continue
