...
class ModelPipeline:
    ...
    def advanced_filter_sentiment(self, df: pd.DataFrame, sentiment_columns: list = None, threshold: float = 0.2, mode: str = "any") -> pd.DataFrame:
        """
        ...
        self.logger.info(f"Advanced sentiment filter: reduced rows from {orig_len} to {len(filtered_df)} (mode='{mode}', threshold={threshold}, columns={valid_cols}).")
        return filtered_df
    ...
    def _drop_future_horizon_columns(self, X: pd.DataFrame, gather_horizon_str: str) -> pd.DataFrame:
        ...
        self.logger.info(f"Dropped {dropped_count} future-horizon columns for gather horizon {gather_horizon_str}. Remaining features: {X_dropped.shape[1]}.")
        return X_dropped
    ...
    def train_on_horizons(
        ...
        if filter_sentiment and sentiment_cols:
            df = self.advanced_filter_sentiment(df, sentiment_columns=sentiment_cols, threshold=sentiment_threshold, mode=sentiment_mode)
            X = X.loc[df.index]
        ...
        for combo in tqdm(combos, desc="Training across horizons", unit="horizon"):
            ...
            if X_f_numeric.shape[1] == 0:
                ...
                continue
            self.logger.info(f"Training with {X_f_numeric.shape[1]} numeric features: {list(X_f_numeric.columns)}")
